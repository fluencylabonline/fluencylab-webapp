...
import { collection, doc, getDocs, updateDoc, onSnapshot, getDoc } from "firebase/firestore";
...
export default function Home() {
  ...
  const [workbookCollectionsState, setWorkbookCollectionsState] = useState<string[]>([]); // You'll need to fetch this
    ...
    async function fetchWorkbooks() {
      try {
        ...
        snapshot.forEach((docSnap) => {
          ...
          if (data.level && data.coverURL) { // Ensure core fields exist
            fetched.push({
              id: docSnap.id,
      ...
      snapshotApostilas.forEach(docSnapApostila => {
        ...
        const unsubscribe = onSnapshot(lessonsColRef, (snapLessons) => {
          ...
          snapLessons.forEach(docLesson => {
            const data = docLesson.data() as Omit<Notebook, 'docID' | 'workbook'>; // Assuming data matches Notebook structure
            notebooks.push({
              ...data,
              docID: docLesson.id,
          ...
          combined.forEach(note => {
            ...
            if (!organized[note.language][note.workbook]) organized[note.language][note.workbook] = [];
  ...
  const handleSaveChanges = async (id: string, newLevel: string, newGuidelines: string, newCoverFile?: File) => {
    const docRef = doc(db, "Apostilas", id);
    let newCoverURL = "";

    if (newCoverFile) {
      const imageRef = ref(storage, `workbookCovers/${uuidv4()}_${newCoverFile.name}`);
    ...
    } else if (newCoverFile) { // Case where only cover was changed, but newCoverURL was set
  ...
  const sortedLevels = Object.keys(grouped).sort(
    (a, b) => levelOrder[a as keyof typeof levelOrder] - levelOrder[b as keyof typeof levelOrder]
  );
    ...
    return (
      <div className="min-h-[90vh] p-8 flex justify-center items-center">
        <div className="flex flex-col items-center">
          <svg className="animate-spin h-12 w-12 text-fluency-blue-500 dark:text-fluency-blue-400" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <>
      <div className="min-h-screen p-4 md:p-8 transition-colors duration-300">
        {/* Search Bar */}
        <div className="flex flex-row items-center gap-6 justify-between mb-6">
          <FluencyInput
            type="text"
          ...
          {session?.user.role === 'teacher' && (
            <FluencyButton variant="warning" onClick={handleOpenCreateModal}>
              Criar
            </FluencyButton>
          )}
        </div>
        ...
        {searchResults.length > 0 ? (
          ...
          >
            <h2 className="text-xl font-semibold mb-4 text-gray-700 dark:text-gray-300">Resultados da busca ({searchResults.length})</h2>
                  ...
                  >
                    <Link
                      href={{
                        ...
                        query: { book: lesson.workbook, lesson: lesson.docID, workbook: lesson.workbook }
                      }}
                      className="block p-4"
                    >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                {grouped[level].map(workbook => (
                  <motion.div
                    ...
                    variants={itemVariants}
                    whileHover={{ scale: 1.05, boxShadow: "0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05)" }} // Add hover effect
                  >
                    <Link
                      href={{ pathname: "material/Apostila", query: { book: workbook.id } }}
                    ...
                    {session?.user.role === 'admin' && (
                      <div className="p-4 flex flex-row items-center justify-between">
                        <div className="flex flex-col items-start">
                          <h3 className="font-medium text-gray-900 dark:text-white line-clamp-2">{workbook.title}</h3>
                          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{workbook.level}</p>
                        </div>
                        <CiEdit onClick={() => handleOpenEditModal(workbook)} className="text-3xl text-gray-500 dark:text-gray-400 hover:text-fluency-blue-600 dark:hover:text-fluency-blue-600 duration-300 ease-in-out cursor-pointer" />
                      </div>)}
                  </motion.div>
                ))}
              </div>
        ...
        {searchQuery.length === 0 && sortedLevels.length === 0 && (
          <div className="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 transition-colors duration-300">
            ...
            <h3 className="mt-2 text-lg font-medium text-gray-900 dark:text-white">Nenhuma apostila encontrada</h3>
            <p className="mt-1 text-gray-500 dark:text-gray-400">Parece que não há apostilas disponíveis no momento.</p>
          </div>
        )}
      </div>
