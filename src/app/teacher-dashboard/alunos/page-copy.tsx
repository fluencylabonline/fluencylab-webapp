...
import { collection, deleteField, doc, FieldValue, getDoc, getDocs, query, updateDoc, where } from 'firebase/firestore';
...
function Alunos(){
    ...
    const handleEditDate = async (studentId: string, oldDate: Date, newDate: Date) => {
        ...
        const year = newParsedDate.getFullYear().toString();
        const month = months[newParsedDate.getMonth()]; // Get the month name from your months array
        const oldDay = oldDate.getDate().toString();
        const newDay = newParsedDate.getDate().toString();
        
        // Construct the reference to the student's document
        ...
        try {
          ...
          if (!studentData?.Classes || !studentData.Classes[year] || !studentData.Classes[year][month]) {
            console.error('Class data for specified year/month does not exist.');
            return;
          }
          
          // Prepare the update data
          const classes = studentData.Classes;
          const monthClasses = classes[year][month] || {};
          
          // Remove the old date
          const { [oldDay]: oldStatus, ...rest } = monthClasses;
          
          // Update with new date
          const updatedMonthClasses = {
            ...rest,
            [newDay]: oldStatus, // Move status from old day to new day
          };
          
          // Update the document
          await updateDoc(studentRef, {
            [`Classes.${year}.${month}`]: updatedMonthClasses,
          });
        } catch (error) {
