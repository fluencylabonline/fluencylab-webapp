...
export default function NivelamentoTeacher() {
    ...
    function generateExcelFile(data: any[], nivel2Data: any[], nivel3Data: Nivel3Data[], userName: any) {
       ...
       /*   ["=(Vocabulario!E2 + Frase!D2 + Compreensao!E2 + VerdadeiroOuFalso!E2)"],
            ["=(Escrita!C2)"],
            ["=(Audicao!B2)"],      */
        ];
        ...
        XLSX.writeFile(workbook, `${userName} - ${userName}.xlsx`);
    }
    ...
    const openDateModal = () => setResetPassword(true);
    const closeDateModal = () => setResetPassword(false);


    const [availableDates, setAvailableDates] = useState<string[]>([]);
const [selectedDate, setSelectedDate] = useState<string>("");

// Function to open the modal and fetch the dates
const openDateSelectionModal = async () => {
    const dates = await fetchReportDates(id);
    setAvailableDates(dates);
    setResetPassword(true); // This opens the modal
};

const handleGenerateReport = () => {
    if (selectedDate) {
        console.log('Generating report for date:', selectedDate);
        closeDateModal(); // Close the modal after generating the report
    } else {
        alert("Please select a date");
    }
};

async function fetchReportDates(userId: any): Promise<string[]> {
    const quizHistoryData = await fetchQuizHistory(userId);
    const nivel2Data = await fetchNivel2Data(userId);
    const nivel3Data = await fetchNivel3Data(userId);

    // Extract dates from each data set
    const quizHistoryDates = quizHistoryData.map((item: any) => item.data);
    const nivel2Dates = nivel2Data.map((item: any) => item.timeStamp);
    const nivel3Dates = nivel3Data.map((item: any) => item.data);

    // Combine all dates into a single array
    let allDates = [...quizHistoryDates, ...nivel2Dates, ...nivel3Dates];

    // Remove duplicates
    allDates = Array.from(new Set(allDates));

    // Sort the dates (optional)
        ...
        <div className="p-4 mt-4 flex flex-col items-center w-full">
            {loading ? (
                ...
                <>
                   ...
                   <FluencyButton variant="gray" onClick={openDateSelectionModal}>Gerar  para {userName}</FluencyButton>
        ...
        <div className="flex items-center justify-center min-h-screen">
            ...
            <div className="bg-fluency-bg-light dark:bg-fluency-bg-dark text-fluency-text-light dark:text-fluency-text-dark rounded-lg overflow-hidden shadow-xl transform transition-all w-fit h-full p-5">
                <div className="flex flex-col items-center justify-center">
                    ...
                    <FluencyCloseButton onClick={closeDateModal}/>
                      ...
                      <div className="mt-2 flex flex-col items-center gap-3 p-4">
                        ...
                        <div className="flex justify-center">
                          <FluencyButton variant='confirm' onClick={handleGenerateReport}>Generate</FluencyButton>
                          <FluencyButton variant='gray' onClick={closeDateModal}>Cancel</FluencyButton>
                        </div>
                      </div>
                </div>
            </div>
