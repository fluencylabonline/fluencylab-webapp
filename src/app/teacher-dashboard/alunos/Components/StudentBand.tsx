...
function Paineis() {
  ...
  const getClassDatesForCurrentMonth = useCallback(
    (diasAula: string[]) => {
  ...
  const handleClassStatus = async (
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        let classes = userData.Classes || {};
        classes[currentYear] = classes[currentYear] || {};
        classes[currentYear][currentMonth] =
          classes[currentYear][currentMonth] || {};
        const dayOfMonth = date.getDate().toString();
    ...
    setStudents((prevStudents) => {
      return prevStudents.map((student) => {
        if (student.id === studentId) {
          const updatedClassDatesWithStatus = student.classDatesWithStatus.map(
            (classDate) => {
          ...
          return {
            ...student,
            classDatesWithStatus: updatedClassDatesWithStatus,
          };
        }
        return student;
      });
  ...
  return (
    <div className="flex flex-col gap-3 lg:items-start md:items-start items-center">
      ...
      {filteredStudents.map((student) => ( 
        <div
          className="bg-fluency-gray-100 dark:bg-fluency-gray-900 w-full lg:flex lg:flex-row md:flex md:flex-row flex flex-col p-3 items-strecht justify-between rounded-lg gap-3"
          key={student.id}
        >
          <div className="bg-fluency-pages-light dark:bg-fluency-gray-800 w-full p-3 px-1 flex flex-col lg:items-start md:items-start items-center justify-around rounded-lg gap-1">
            <div className="lg:flex lg:flex-row md:flex md:flex-row flex flex-col lg:items-start md:items-start items-center lg:gap-3 md:gap-2 gap-0">
              ...
              <div className="flex flex-col lg:items-start md:items-start items-center lg:mb-0 md:mb-0 mb-2">
                <p className="font-semibold text-lg lg:text-start md:text-start text-center">
                  {student.name}
                </p>
                <div className="flex flex-col gap-1 items-start">
                  <p className="font-bold text-xs bg-fluency-blue-400 dark:bg-fluency-blue-700 px-2 py-[2px] rounded-md text-white">
                    Dias de aula:
                    {student.diaAula?.map((dia) => (
                      <span className="ml-1" key={dia}>
                        {dia}{" "}
                      </span>
                    ))}
                  </p>
            ...
            <div className="flex flex-col gap-2 items-center w-full mb-8 font-medium">
              <div className="lg:flex lg:flex-row md:flex md:flex-row flex flex-col items-center justify-center gap-1">
                <Link
                  ...
                  passHref
                >
                  <FluencyButton variant="glass" className="!px-3 !mr-0">
                ...
                <div className="flex flex-row items-center justify-center gap-1">
                  <Link
                    ...
                    passHref
                  >
                    <FluencyButton variant="glass" className="!px-2.5 !mr-0">
                      <TbBook2 className="w-5 h-auto" />
                    </FluencyButton>
                  </Link>
                  <SettingBottomSheet
          ...
          <div className="bg-fluency-pages-light dark:bg-fluency-gray-800 p-2 px-4 flex flex-col items-center rounded-lg gap-2 w-full">
                ...
                <div className="lg:flex lg:flex-row md:flex md:flex-row flex flex-col items-center gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const updatedStudents = students.map((s) =>
                        s.id === student.id
                          ? { ...s, selectedYear: parseInt(e.target.value) }
                          : s
                      );
                      setStudents(updatedStudents);
                    }}
            ...
            {student.classDatesWithStatus
              .filter((classDate) => {
                ...
                return (
                  dateMonth === student.selectedMonth &&
                  dateYear === student.selectedYear
                );
              })
              .slice(
                0,
                student.showAllDates ? student.classDatesWithStatus.length : 4
              )
              .map((classDate, index) => (
                <ClassDateItem
                  key={index}
