...
interface VocabularyPlacementProps {
  onClose: () => void;
  testId: any;
}

const VocabularyPlacement: React.FC<VocabularyPlacementProps> = ({ onClose, testId }) => {
  ...
  useEffect(() => {
    async function fetchVocabularyTest() {
      if (!userId) return;
      setLoading(true);
      try {
        ...
        if (docSnap.exists()) {
          const vocabularyData: VocabularyQuestion[] = docSnap.data()?.vocabulary;
          if (Array.isArray(vocabularyData) && vocabularyData.length > 0) {

            // Calculate total difficulty
            const totalDifficulty = vocabularyData.reduce((sum: number, q: any) => {
            ...
            const calculatedPoints = vocabularyData.map((q: any) => {
            ...
            if (pointsToAdjust !== 0) {
              ...
              let questionsSortedByDifficulty = vocabularyData.map((q: any, index: number) => ({ difficulty: q.question.difficulty, index: index }))
            ...
            setVocabularyQuestions(vocabularyData.map((q: any, index: number) => ({ ...q, points: adjustedPoints[index] }))); // Add points to questions
            setQuestionPoints(adjustedPoints);


            // Calculate first incomplete question
            const firstIncompleteIndex = vocabularyData.findIndex(q => !q.completed);
            setCurrentIndex(firstIncompleteIndex !== -1 ? firstIncompleteIndex : vocabularyData.length - 1);
          }
        } else {
  ...
  const getVocabularyPrompt = (item: VocabularyQuestion["question"]) => {
    if (!item) return "";
    switch (item.type) {
  ...
  const saveVocabularyResults = useCallback(async (updatedQuestions: VocabularyQuestion[]) => {
    ...
    try {
      ...
      await updateDoc(placementDocRef, {
        vocabulary: updatedQuestions,
        "abilitiesScore.vocabularyScore": totalScore,
        "abilitiesCompleted.vocabularyCompleted": allCompleted,
      });
  ...
  const handleOptionSelect = (option: string) => {
    ...
    if (!vocabularyQuestions[currentIndex].completed) {
  ...
  const handleConfirmAnswer = async () => {
    ...
    const currentQuestion = vocabularyQuestions[currentIndex];
    const isCorrect = selectedOption === currentQuestion.question.correct;
    const updatedQuestion: VocabularyQuestion = {
      ...currentQuestion,
      answer: selectedOption,
      completed: true,
      score: isCorrect ? 1 : 0, // Score 1 if correct, 0 if incorrect
    };
    const updatedQuestions = vocabularyQuestions.map((q, i) =>
      i === currentIndex ? updatedQuestion : q
    );
    setVocabularyQuestions(updatedQuestions);
    setSelectedOption(null);
    await saveVocabularyResults(updatedQuestions);
    if (currentIndex < vocabularyQuestions.length - 1) {
      handleNext();
    } else {
      onClose();
      router.refresh();
  ...
  const handleSkip = async () => {
    const updatedQuestion: VocabularyQuestion = {
      ...vocabularyQuestions[currentIndex],
      answer: null,
      completed: true,
  ...
  const currentQuestion = vocabularyQuestions[currentIndex];
  const promptText = getVocabularyPrompt(currentQuestion.question);
            ...
            <div className="mt-6 flex justify-center space-x-4">
              ...
              {currentQuestion.completed && (
                <FluencyButton
                  ...
                  variant="purple"
                >
                  {currentIndex < vocabularyQuestions.length - 1 ? "Próxima Pergunta" : "Finalizar Teste"}
                </FluencyButton>
              )}
            </div>
            <div className="text-gray-600 dark:text-gray-500 mt-2">
              {`Pergunta ${currentIndex + 1} de ${vocabularyQuestions.length}`}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
export default VocabularyPlacement;