            ...
            {lessonDocs.map((group, index) => (
                <div key={index}>
                  ...
                  <AccordionItem key={index} aria-label={group.unit.toString()} title={"Unidade " + group.unit.toString()}>
                    ...
                    <div className="flex flex-col items-center justify-center text-center w-28 h-40 bg-fluency-bg-light dark:bg-fluency-bg-dark p-4 rounded-sm break-all" key={lessonIndex}>
                        <Link key={lesson.id} href={{ pathname: `apostilas/${encodeURIComponent(lesson.title)}`, query: { workbook: lesson.workbook, lesson: lesson.id }}} ><p className="font-bold break-all hover:text-fluency-blue-500 duration-300 ease-in-out cursor-pointer">{lesson.data.title}</p></Link>
                    </div>
                    ))}
                      </div>
                    </AccordionItem>
