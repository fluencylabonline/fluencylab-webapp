...
export default function PodcastPage() {
  ...
  const fetchPodcasts = async (isLoadMore = false) => {
    setLoading(true)
    try {
      let q = query(collection(db, 'podcasts'), orderBy('createdAt', 'desc'), limit(6))
      ...
      if (session?.user?.id) {
        const progressPromises = docs.map(async (podcast) => {
          const progressDocRef = doc(db, 'users', session.user.id!, 'podcastProgress', podcast.id);
          ...
          return { ...podcast, resumeTime: null, completed: false, hasResumed: false }; // No progress found
        });
  ...
  const filtered = podcasts.filter((p) => {
    return (
      ...
      (!search ||
        ...
        (p.description && p.description.toLowerCase().includes(search.toLowerCase())) || // Check if description exists
        (p.transcription && p.transcription.toLowerCase().includes(search.toLowerCase())) // Check if transcription exists
      )
    )
  })
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 pb-32 pt-2 relative">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
        ...
        <select
          className="bg-fluency-pages-light dark:bg-fluency-pages-dark px-4 py-3 rounded-lg border border-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white"
       ...
       {/* Podcast Cards */}
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Nenhum podcast encontrado.</h3>
            <p className="text-gray-600">Tente ajustar seus filtros ou pesquise termos.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      {nowPlaying && (
        <div
          className={`fixed bottom-0 inset-x-0 shadow z-40 transition-all duration-300 ${
