...
export default function ModuloTres() {
   ...
   const closeEditClass = () => {
    ...
    const handleEditSubmit = async () => {
        try {
            const classRef = doc(db, 'Modulos', 'Ingles', 'Modulo-3', editClass!.id);
    ...
    const handleSubmit = async () => {
        try {
            ...
            const newClassRef = doc(collection(moduleRef, 'Modulo-3')); // Replace with actual dynamic ID generation
    ...
    const fetchClasses = async () => {
        try {
            const moduleRef = collection(db, 'Modulos', 'Ingles', 'Modulo-3');
    ...
    const handleDeleteClass = async () => {
        try {
            if (classToDelete) {
                const classRef = doc(db, 'Modulos', 'Ingles', 'Modulo-3', classToDelete.id);
    ...
    const handleAddDeckAsTask = async (studentId: string, className: string, id: string) => {
        try {
            ...
            const deckLink = `/student-dashboard/aulas-gravadas/modulo-3/${encodeURIComponent(className)}?id=${id}`;
            ...
            <div className="flex flex-row flex-wrap items-center justify-center w-full gap-2">
                {filteredClasses.map((classData, index) => (
                    <div key={index} className="relative">
                        <Link href={{ pathname: `modulo-3/${encodeURIComponent(classData.className)}`, query: { id: classData.id } }} passHref>
