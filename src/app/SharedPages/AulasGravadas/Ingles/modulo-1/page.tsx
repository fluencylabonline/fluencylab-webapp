...
export default function ModuloUm() {
   ...
   const closeEditClass = () => {
    ...
    const handleEditSubmit = async () => {
        try {
            const classRef = doc(db, 'Modulos', 'Ingles', 'Modulo-1', editClass!.id);
    ...
    const handleDeleteClass = async () => {
        try {
            if (classToDelete) {
                const classRef = doc(db, 'Modulos', 'Ingles', 'Modulo-1', classToDelete.id);
    ...
    const handleAddDeckAsTask = async (studentId: string, className: string, id: string) => {
        try {
            ...
            const deckLink = `/student-dashboard/aulas-gravadas/modulo-1/${encodeURIComponent(className)}?id=${id}`;
