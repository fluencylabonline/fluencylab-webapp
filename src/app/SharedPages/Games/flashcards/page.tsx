...
const FlashCard: FC = () => {
  ...
  const [globalDecksPractice, setGlobalDecksPractice] =
  ...
  useEffect(() => {
    const filtered = otherDecks.filter(
      (deck) =>
  ...
  useEffect(() => {
    if (selectedDeck && isModalOpen) {
      // --- MODIFIED --- Fetch only if modal is open to avoid unnecessary fetches
  ...
  const addCard = async () => {
    if (selectedDeck && newCardFront && newCardBack) {
      try {
        const newCardData = {
          // --- MODIFIED --- Use specific fields for global cards
          front: newCardFront,
          back: newCardBack,
        };
  ...
  const updateCard = async () => {
    if (selectedDeck && editedCardId && (editedCardFront || editedCardBack)) {
      // --- MODIFIED --- ensure front/back are not empty
      try {
  ...
  const deleteCard = async (cardId: string) => {
    if (selectedDeck) {
      // --- MODIFIED --- ensure selectedDeck is present
      try {
        const cardRef = firestoreDoc(
          db,
          "Flashcards",
  ...
  const reviewCard = async (
    ...
    switch (rating) {
      ...
      case "medium":
        interval =
          reviewCount === 0
            ? interval * 2.5
            : interval * (easeFactor - 0.1 < 1.3 ? 1.3 : easeFactor - 0.1); // Adjust interval
        break;
      case "hard":
    ...
    ).toISOString(); // Ensure at least 1 day
  ...
  const [isOtherConfirmModalOpen, setIsOtherConfirmModalOpen] =
    useState<boolean>(false);
  ...
  const addSelectDeck = async (deckId: string, targetUserId: string) => {
    // Renamed studentId to targetUserId
    try {
      ...
      if (globalDeckSnapshot.exists()) {
        ...
        await setDoc(userDeckRef, {
          name: globalDeckData.name,
          tags: globalDeckData.tags || [],
        }); // Copy relevant deck info
        ...
        toast.success(
          `Deck '${globalDeckData.name}' adicionado! Recarregue a página para praticar ou ver na sua lista.`
        );
        if (targetUserId === currentUserId) {
          // If adding to current user, update their decks list
          const updatedUserDecks = await fetchDecksWithReviewCount(
            targetUserId
          );
  ...
  const handleImportFromCSV = async () => {
    ...
    try {
      ...
      if (deckDocSnapshot.exists()) {
        toast.error(
          `Um deck chamado '${deckNameFromCSV}' já existe. Renomeie o arquivo CSV ou o deck existente.`
        );
      ...
      Papa.parse(selectedFile, {
        ...
        complete: async (results) => {
          ...
          if (validCards.length !== parsedCards.length) {
            toast.error(
              "Alguns cartões no CSV não tinham 'Front' ou 'Back' e foram ignorados."
            );
          }
          if (validCards.length === 0) {
            toast.error(
              "Nenhum cartão válido encontrado no CSV. Certifique-se de que as colunas 'Front' e 'Back' existem e estão preenchidas."
            );
            setIsUploading(false);
            return;
          }
          ...
          toast.success(
            `Deck '${deckNameFromCSV}' importado com ${validCards.length} cartões!`
          );
      ...
      toast.error(
        `Erro ao importar deck do CSV: ${error.message || "Erro desconhecido"}`
      );
      setIsUploading(false);
    }
  };
  ...
  const deckName =
    ...
    otherDecks.find((deck) => deck.id === selectedDeck)?.name;
                ...
                <div className="flex flex-col items-center">
                  ...
                  {decks.length === 0 && (
                    <p className="text-gray-500 mt-4">
                      Você ainda não tem decks para praticar. Explore a aba
                      Decks!
                    </p>
                  )}
                  <ul className="flex flex-col items-start p-4 gap-2">
                    {decks.map((deck) => (
                      <li
                        id="deck-bg"
                        className="flex flex-col items-center gap-6 p-2 py-8 px-3 rounded-lg w-full justify-between"
                        key={deck.id}
                      >
                        <p className="font-bold px-4">{deck.name}</p>
                        <div className="flex flex-col sm:flex-row gap-6 items-center">
                          <div
                            className={`flex font-bold text-sm p-1 rounded-md ${
                          ...
                          <button
                            disabled={deck.cardsToReviewCount === 0}
          ...
          <Tab key="disponiveis" title="Decks">
            <div className="flex flex-col items-center">
              ...
              {filteredDecks.length === 0 && (
                <p className="text-gray-500 mt-4">
                  Nenhum deck global encontrado ou correspondente à busca.
                </p>
              )}
              <ul className="flex flex-col items-start p-4 gap-2 w-full">
                {" "}
                {/* --- MODIFIED --- Added w-full */}
                {filteredDecks.length > 0 &&
                        ...
                        {deck.tags && deck.tags.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {deck.tags.map((tag) => (
                              <span
                                key={tag}
                                className="text-xs bg-fluency-blue-200 dark:bg-fluency-blue-800 px-2 py-0.5 rounded-full"
                              >
                                {tag}
                              </span>
                            ))}
              ...
              <div>
                <div className="flex flex-col items-center gap-2 p-2 rounded-md">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 my-2">
                    Ou atribua um deck global existente para um aluno:
                  </p>
                  <FluencyInput
                    type="text"
                  ...
                  />
                  <div className="flex flex-col items-center gap-1 h-fit max-h-[55vh] overflow-y-auto w-full px-4">
                    {" "}
                    {/* --- MODIFIED --- Added w-full px-4 */}
                    {filteredDecks.length === 0 && (
                      <p className="text-gray-500 mt-4">
                        Nenhum deck global para atribuir.
                      </p>
                    )}
                    <ul className="flex flex-col items-start py-2 gap-2 w-full">
                      {filteredDecks.map((deck) => (
                        <li
                          className="flex flex-row items-center gap-6 p-2 px-3 rounded-md bg-fluency-pages-light dark:bg-fluency-bg-dark w-full justify-between"
                          key={deck.id}
                        >
                          <p className="font-bold">{deck.name}</p>
                          <div className="flex flex-row gap-2 items-center">
                            <button
                              className="bg-fluency-orange-500 hover:bg-fluency-orange-600 dark:bg-fluency-orange-700 hover:dark:bg-fluency-orange-800 text-white font-semibold text-sm p-2 rounded-md duration-300 transition-all ease-in-out"
                              value={deck.name}
                              onClick={() => openConfirmModal(deck.id)}
                            >
                              Enviar para Aluno
                            </button>
                          </div>
            ...
            <div className="bg-fluency-bg-light dark:bg-fluency-bg-dark text-fluency-text-light dark:text-fluency-text-dark rounded-lg overflow-hidden shadow-xl transform transition-all w-full h-[95vh] p-3 m-5">
              <div className="flex flex-col items-center h-full">
                ...
                <div className="w-full p-4 border-b border-gray-300 dark:border-gray-700 mb-4">
                  <h4 className="text-lg leading-6 font-medium mb-2">
                    Importar Deck de CSV
                  </h4>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">
                    O arquivo CSV deve ter colunas Front e Back. O nome do
                    arquivo (sem extensão) será usado como nome do deck.
                  </p>
                ...
                <div className="flex flex-col lg:flex-row items-start w-full justify-around gap-4 overflow-y-auto flex-grow">
                  {" "}
                  {/* --- MODIFIED --- flex-grow */}
                  <div className="flex flex-col gap-3 items-center w-full lg:w-1/2 h-full bg-fluency-pages-light dark:bg-fluency-pages-dark p-2 rounded-md overflow-y-auto">
                    <h4 className="text-lg leading-6 font-medium">
                      Criar Deck Manualmente ou Editar Tags
                    </h4>
                    <div className="flex flex-col gap-2 w-full items-start">
                      ...
                      >
                        <option value="">
                          Selecione um deck para editar tags ou adicionar
                          cartões abaixo
                        </option>
                        {otherDecks.map((deck) => (
                    ...
                    {selectedDeck && ( // Only show manual card add if a deck is selected
                      <div className="w-full mt-4 border-t pt-4">
                        <h4 className="text-md leading-6 font-medium mb-2">
                          Adicionar Cartão Manualmente ao Deck {selectedDeck}
                        </h4>
                        <FluencyInput
                          type="text"
                          value={newCardFront}
                  ...
                  <div className="flex flex-col items-center w-full lg:w-1/2 h-full bg-fluency-pages-light dark:bg-fluency-pages-dark rounded-md p-2">
                    <h4 className="text-lg leading-6 font-medium">
                      Cartões no deck {selectedDeck || "Nenhum selecionado"}
                    </h4>
                    {!selectedDeck && (
                      <p className="text-sm text-gray-500 mt-4">
                        Selecione um deck à esquerda para ver seus cartões.
                      </p>
                    )}
                    {selectedDeck && editedCardId && (
                      <div className="flex flex-row gap-3 items-center w-full my-2 p-2 border rounded-md dark:border-gray-700">
                        ...
                        <FluencyInput
                          ...
                          placeholder="Fundo do Cartão"
                        />
                        <FluencyButton onClick={updateCard}>
                          Salvar
                        </FluencyButton>
                        <FluencyButton
                          variant="danger"
                    ...
                    <ul className="w-full h-full overflow-y-scroll p-1 flex flex-col gap-2">
                      {selectedDeck && otherCards.length === 0 && (
                        <p className="text-sm text-gray-500 mt-4">
                      ...
                      {otherCards.map((card) => (
                        <li
                          key={card.id}
                          className="flex flex-col lg:flex-row items-start lg:items-center border-b border-gray-200 dark:border-gray-700 pb-2 mb-2"
                        >
                            ...
                            <div className="flex justify-between items-center">
                              ...
                              {editedCardId !== card.id && (
                                <div className="flex gap-1">
                                  <Tooltip
                                    className="bg-fluency-blue-500 font-bold text-sm text-white p-1 rounded-md"
                                    content="Editar cartão"
                                  >
                                    <button
                                      className="bg-fluency-blue-600 hover:bg-fluency-blue-700 p-1.5 text-white rounded-md"
                                      onClick={() =>
                                        handleEditInputChange(card)
                                      }
                                    >
                                      <FiEdit3 size={14} />
                                    </button>
                                  ...
                                  <Tooltip
                                    className="bg-fluency-red-500 font-bold text-sm text-white p-1 rounded-md"
                                    content="Deletar cartão"
                                  >
                                    <button
                                      className="bg-fluency-red-600 hover:bg-fluency-red-700 p-1.5 text-white rounded-md"
      ...
      {isOtherConfirmModalOpen && (
        <div className="fixed z-50 inset-0 overflow-y-auto">
          <div className="flex items-center justify-center min-h-screen">
            <div
              className="fixed inset-0 transition-opacity"
              onClick={closeOtherConfirmModal}
            >
              <div className="absolute inset-0 bg-gray-500 opacity-75"></div>
            </div>
            <div className="bg-fluency-bg-light dark:bg-fluency-bg-dark text-fluency-text-light dark:text-fluency-text-dark rounded-lg overflow-hidden shadow-xl transform transition-all w-fit h-fit p-5">
              {" "}
              {/* --- MODIFIED --- h-fit */}
              <div className="flex flex-col">
                ...
                <div className="mt-3 flex flex-col gap-3 p-4">
                  <h3 className="text-center text-lg leading-6 font-bold mb-2">
                    ...
                    {otherDecks.find((d) => d.id === selectedDeck)?.name ||
                      selectedDeck}{" "}
                    aos seus decks pessoais para estudo?
                  </h3>
              ...
              <div className="flex flex-col">
                ...
                <div className="mt-3 flex flex-col gap-3 p-4">
                  ...
                  <div className="flex justify-center gap-2">
                    ...
                    <FluencyButton
                      variant="confirm"
                      onClick={confirmDeckAddition}
                    >
                      Sim, quero enviar.
                    </FluencyButton>
                    <FluencyButton variant="gray" onClick={closeConfirmModal}>
                      Não, cancelar
                    </FluencyButton>
                  </div>
                </div>
