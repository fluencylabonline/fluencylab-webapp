        ...
        try {
            if (session?.user?.id) {
                ...
                const cardsData = cardsSnapshot.docs.map(doc => ({
                    ...
                    ...doc.data(),
                    review: doc.data().review || 0,
                    nextReviewDate: doc.data().nextReviewDate || Timestamp.now().toDate().toISOString()
                }) as Card);
            ...
            if (session?.user?.id) {
                ...
                switch (status) {
                    case 'easy':
                        switch (reviewCount) {
                            case 1:
                                nextReviewDate = new Date(currentTime.toMillis() + 1 * 24 * 60 * 60 * 1000); // 1 day later
                                break;
                            case 2:
                                nextReviewDate = new Date(currentTime.toMillis() + 3 * 24 * 60 * 60 * 1000); // 3 days later
                                break;
                            case 3:
                                nextReviewDate = new Date(currentTime.toMillis() + 7 * 24 * 60 * 60 * 1000); // 7 days later
                                break;
                            default:
                    ...
                    case 'medium':
                        switch (reviewCount) {
                            ...
                            case 2:
                                ...
                                break;
                            case 3:
                                nextReviewDate = new Date(currentTime.toMillis() + 14 * 24 * 60 * 60 * 1000); // 14 days later
                                break;
                            default:
                                nextReviewDate = null; // Handle other cases if necessary
                                break;
                    ...
                    case 'hard':
                        switch (reviewCount) {
                            ...
                            case 2:
                                ...
                                break;
                            case 3:
                                nextReviewDate = new Date(currentTime.toMillis() + 30 * 24 * 60 * 60 * 1000); // 30 days later
                                break;
                            default:
                                nextReviewDate = null; // Handle other cases if necessary
                                break;
                ...
                // Convert nextReviewDate to Firebase Timestamp if it's not null
                const nextReviewTimestamp = nextReviewDate ? Timestamp.fromDate(nextReviewDate) : null;
