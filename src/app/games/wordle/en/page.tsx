...
const Wordle = () => {
  ...
  useEffect(() => {
    ...
    const storedGuesses = localStorage.getItem('english_wordle_guesses');
    if (storedGuesses) {
    ...
    const storedDate = localStorage.getItem('english_wordle_last_played_date');
  ...
  useEffect(() => {
    ...
    localStorage.setItem('english_wordle_guesses', JSON.stringify(guesses));
  }, [guesses]);
    ...
    const playOncePerDay = () => {
      ...
      if (lastPlayedDate === currentDate) {
        ...
        if ((isGuessed || currentRow === 5)) {
          localStorage.setItem('english_wordle_last_played_date', currentDate);
        }
      }
    };
  ...
  useEffect(() => {
    const storedenglish_wordAndDate = localStorage.getItem('english_wordAndDate');
    if (storedenglish_wordAndDate) {
      const { word, date } = JSON.parse(storedenglish_wordAndDate);
  ...
  const chooseWordOfTheDay = () => {
    ...
    const today = new Date().toLocaleDateString();
    localStorage.setItem('english_wordAndDate', JSON.stringify({ word: randomWord, date: today }));
  };
      ...
      if (lastPlayedDate === new Date().toLocaleDateString()) {
        return (
          <div className='lg:mt-4 md:mt-1 gap-4 flex flex-col items-center bg-blue-different dark:bg-black-different'>
            ...
            <h1 className='text-2xl font-bold text-black dark:text-white'>You already played!</h1>
            <p className="text-center w-[75%] text-black dark:text-white">If you want to play more times, sign up to one of our courses. <br /> You will have a lot more games and even more help to learn another language!</p>
              ...
              <div className='flex flex-col items-center gap-2 text-black dark:text-white'>
                ...
                <p>or, if you are already one of our students:</p>
                <div>
  ...
  return (
    <div className='h-[100vh] bg-fluency-pages-light dark:bg-fluency-pages-dark text-fluency-text-light dark:text-fluency-text-dark'>
      {/* DEFAULT HEADER OF GAMES*/}
      <div>
        <div className='w-full flex flex-row gap-3 justify-between px-4 py-3 items-center absolute top-0'>
          ...
          <div className='flex flex-row items-center justify-around gap-4'>
            ...
            <CiCircleQuestion className='lg:w-7 lg:h-7 w-5 h-5 text-yellow-700 cursor-pointer' onClick={toggleInstructions} />
          </div>
