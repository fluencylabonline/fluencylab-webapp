...
export default function Lista() {
    ...
    useEffect(() => {
        const unsubscribe = onSnapshot(getQuery(), async (snapshot) => { // Made async to fetch contract status
            const updatedAlunos: Aluno[] = [];
            for (const docSnapshot of snapshot.docs) { // Use for...of with await for async operations
                const aluno: Aluno = {
                    id: docSnapshot.id,
  ...
  {aluno.ContratosAssinados?.signed ? (
    <Tooltip content={`Assinado em ${new Date(aluno.ContratosAssinados.signedAt!).toLocaleDateString('pt-BR', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
      hour: '2-digit',
      ...
      <span className="text-green-600 font-bold">✔️ Sim</span>
    </Tooltip>
  ) : (
