...
export default function SignIn(){
  ...
  async function handleSignIn() {
    ...
    setIsLoginLoading(false); // Set login loading state to false after login completes

    if (signInResponse?.ok) {
      setIsDashboardLoading(true); // Set dashboard loading state to true after successful login
      const session = await getSession(); // Get the session
      if (session?.user?.role) {
        // Check if role is available in session.user
          ...
          <div className="md:flex w-full">
              ...
              <div className="w-full md:w-1/2 py-10 px-5 md:px-10">
                  <div className="text-center mb-10">
                    ...
                    <p className='text-fluency-text-light dark:text-fluency-text-dark'>Bem-vindo de volta!</p>
                  </div>
                <div>
