...
function Perfil() {
        ...
        if (session && session.user && session.user.id) {
          try {
            ...
            if (docSnap.exists()) {
              ...
              // Ensure 'meioPagamento' has a valid value (either 'pix' or 'cartao')
              setMeioPagamento(docSnap.data().meioPagamento || 'pix');  // Default to 'pix' if not set
    
              setMensalidade(docSnap.data().mensalidade);
            } else {
  ...
  const [contratoFoiAssinado, setContratoFoiAssinado] = useState<{ 
    ...
    <div className="flex flex-col items-center lg:pr-2 md:pr-2 pt-3 bg-fluency-bg-light dark:bg-fluency-bg-dark text-fluency-text-light dark:text-fluency-text-dark">                   
        <div className='fade-in fade-out w-full lg:flex lg:flex-row md:flex md:flex-col sm:flex sm:flex-col overflow-y-auto h-[90vh]'>
                    ...
                    {meioPagamento === 'pix' && (
                      <div>
                        <p className='text-justify'>
                          Você pode fazer a transferência para as seguintes chaves PIX ou simplesmente usar o QR Code:
                        </p>
          ...
          <div className='lg:flex lg:flex-row lg:items-stretch flex flex-col items-stretch justify-center w-full gap-4 lg:mt-0 mt-2 lg:ml-4 ml-0'>
